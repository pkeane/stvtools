...
foreach (file('new_condorcet_simulation_count') as $line) {
		...
		$ballots = json_decode(file_get_contents($file));
		...
		foreach (range(1,100) as $num) {
				if (0 == $num%10) {
					//	print $num." memory ".memory_get_usage()."\n";
				}
		...
		$stv_committee = array();
		foreach ($committee as $eid => $tally) {
				$stv_committee[] = $eid.":".$tally;
		}
		$stv_winners = array_keys($committee);

		print $file." condorcet:".$winner." STV:".join(',',$stv_committee);
		if (in_array($winner,$stv_winners)) {
				$place = $committee[$winner];
				if (1 == count($committee)) {
						$flag = "EQUAL";
				} else {
						$flag = "YES IN STV SET - place: $place";
				}
		} else {
				$flag = "NO IN STV SET - place: NOT";
		}
		print " ".$flag."\n";
