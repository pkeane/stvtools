...
function findApproval($ballots,$seats) {
		$tallies = array();
		foreach ($ballots as $ballot) {
				$ballot_length = count($ballot);
				foreach (range(0,$ballot_length-1) as $num) {
						if (isset($ballot[$num])) {
								$cname = $ballot[$num];
